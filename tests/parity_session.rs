...
use cusage_rs::report::{build_session_report, render_session_report_json};
use std::fs;
use std::path::{Path, PathBuf};

#[test]
fn session_json_matches_basic_fixture() {
    assert_fixture("basic");
}

#[test]
fn session_json_handles_malformed_lines_deterministically() {
...
fn assert_fixture(fixture_name: &str) {
    ...
    let first = run_session_json(&fixture_root.join("claude-config"));
    let second = run_session_json(&fixture_root.join("claude-config"));
...
fn run_session_json(claude_config_dir: &Path) -> String {

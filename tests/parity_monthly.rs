...
use cusage_rs::report::{build_monthly_report, render_monthly_report_json};
use std::fs;
use std::path::{Path, PathBuf};

#[test]
fn monthly_json_matches_basic_fixture() {
    assert_fixture("basic");
}

#[test]
fn monthly_json_handles_malformed_lines_deterministically() {
...
fn assert_fixture(fixture_name: &str) {
    ...
    let first = run_monthly_json(&fixture_root.join("claude-config"));
    let second = run_monthly_json(&fixture_root.join("claude-config"));
...
fn run_monthly_json(claude_config_dir: &Path) -> String {

...
use cusage_rs::report::{build_daily_report, render_daily_report_json};
use std::fs;
use std::path::{Path, PathBuf};

#[test]
fn daily_json_matches_basic_fixture() {
    assert_fixture("basic");
}

#[test]
fn daily_json_handles_malformed_lines_deterministically() {
...
fn assert_fixture(fixture_name: &str) {
    ...
    let first = run_daily_json(&fixture_root.join("claude-config"));
    let second = run_daily_json(&fixture_root.join("claude-config"));
...
fn run_daily_json(claude_config_dir: &Path) -> String {
